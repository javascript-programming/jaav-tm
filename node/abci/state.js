class State {
    ...
    constructor (mongo) {
        ...
        me.insertRecord = async (record, collection) => {
            return new Promise((resolve, reject) => {
                mongo.database.collection(collection).insert(record, { session: this.session }).then(resolve).catch(reject)
            });
        };

        me.updateRecord = async (id, update, collection) => {
            return new Promise((resolve, reject) => {
                mongo.database.collection(collection).update({ _id : id},{ $set: update }, { session: this.session }).then(resolve).catch(reject)
            });
        };
    }
}
